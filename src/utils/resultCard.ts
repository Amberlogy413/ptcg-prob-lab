...
export function buildResultCardSvg(spec: ResultCardSpec): string {
  ...
  <rect x="24" y="24" width="${W - 48}" height="${H - 48}" rx="10" fill="#FFFFFF" stroke="#E4E7EC"/>
  <text x="48" y="76" font-family="${sans}" font-size="20" fill="#15181C">${esc(spec.title)}</text>
  <text x="48" y="100" font-family="${sans}" font-size="14" fill="#5F6976">${esc(spec.conditionLabel)}</text>
  <text x="48" y="208" font-family="${mono}" font-size="72" fill="#15181C">${esc(spec.percent)}</text>
  <text x="48" y="248" font-family="${mono}" font-size="22" fill="#5F6976">${esc(spec.fraction)} · ${esc(spec.oneIn)}</text>
  <line x1="48" y1="284" x2="${W - 48}" y2="284" stroke="#E4E7EC"/>
  <rect x="48" y="304" width="190" height="30" rx="6" fill="none" stroke="#3B4658"/>
  <text x="143" y="324" font-family="${sans}" font-size="13" fill="#3B4658" text-anchor="middle">${esc(spec.badge)}</text>
  <text x="48" y="366" font-family="${sans}" font-size="15" fill="#15181C">${esc(spec.product)}</text>
  <text x="${W - 48}" y="366" font-family="${sans}" font-size="11" fill="#5F6976" text-anchor="end">${esc(spec.footer)}</text>
</svg>`;
}
