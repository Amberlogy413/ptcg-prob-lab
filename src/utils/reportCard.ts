...
export function buildReportCardSvg(
  ...
  for (const line of lines) {
    y += rowH;
    parts.push(
      `<line x1="40" y1="${y - rowH + 6}" x2="${W - 40}" y2="${y - rowH + 6}" stroke="#E4E7EC"/>`,
      `<text x="40" y="${y - 6}" font-family="${SANS}" font-size="14" fill="#5F6976">${esc(line.label)}</text>`,
  ...
  parts.push(
    ...
    `<text x="125" y="${fy - 1}" font-family="${SANS}" font-size="12" fill="#3B4658" text-anchor="middle">${esc(chrome.badge)}</text>`,
    `<text x="226" y="${fy - 1}" font-family="${SANS}" font-size="14" fill="#15181C">${esc(chrome.product)}</text>`,
    `<text x="${W - 40}" y="${fy - 1}" font-family="${SANS}" font-size="11" fill="#5F6976" text-anchor="end">${esc(chrome.footer)}</text>`,
    `</svg>`,
  );
