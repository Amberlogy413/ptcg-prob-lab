...
任務:
1. **P11.1 賽事數據管道**:外部工具腳本(如 catalog 管道)抓取公開賽事結果
   (Limitless 等公開來源)→ 靜態快照 JSON(牌組原型、三甲名單、卡片使用率);
   無後端、無爬蟲進產品,數據隨 build 出貨、註明快照日期與來源。
2. **P11.2 熱門統計視圖**:熱門卡片使用率(可按原型篩選)、熱門牌組原型分布、
   真實比賽三甲牌組一覽;每個原型一鍵載入為本品牌組 → 即時體檢/組牌。
3. **P11.3 組牌器整合**:卡片格仔顯示「使用率」徽章;「熱門」排序選項。

DoD:統計頁標明快照日期/來源/樣本數;原型載入後 60 張齊全、黃金測試綠;
數據更新 = 重跑管道 + 重新部署,流程記錄於 DECISIONS。
