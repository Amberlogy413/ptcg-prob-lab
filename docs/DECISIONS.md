...
## 2026-06-16 — 白話詳解:每個數字第二個「?」掣,極致詳盡人話解釋

- **需求**:產品負責人要喺每個主要概率數字側邊「再新增一個細細粒符號」,㩒開彈出最詳盡嘅
  逐步計算明細,並用人話極致詳盡解釋所有代數符號嘅意思同意義。
- **實作**:ProofNumber 加第二個圓形「?」掣(HelpGlyph,喺 √ 收據掣隔籬)→ 開一個
  「白話詳解」模態:五段 plain-language(呢個數答緊咩 → 啲符號代表咩 → 點樣一步步計 →
  個答案點睇 → 點解信得過)+ 底下附返精確收據(代入真實數字)+ 複製。內容由
  `src/data/explain.ts` `buildExplain(t, kind, {pct,frac,oneIn})` 砌,kind =
  mulligan/combo/prize/midgame/draw;結構意義係通用真話,具體數字交俾收據(避免重複又
  確保準確)。i18n `explain.*`(zh 廣東話 + en)。
- **鋪開**:重抽(DeckSummary 側欄 / Q1 / 體檢)、Q2 組合、Q3 單張、體檢組合、中局 ×3、
  對戰 HUD。Q3 聯合(多卡多變量,niche)暫只保留 √,唔貼一個唔夠貼切嘅解釋(誠實 > 覆蓋)。
- **守則**:解釋全部係事實(規則 + 超幾何結構 + 三種寫法 + BigInt/黃金精確保證),零老作;
...
## 2026-06-16 — 牌組推薦配色改用「副牌真實能量屬性」+ 能量屬性 icon

- **問題(產品負責人指出)**:用主軸寶可夢屬性上色,但揀錯卡/錯色——厄鬼椪有 4 個面具
  (4 屬性),我用淨種名 resolveDeckRow 揀錯;祭典樂舞主軸裹蜜蟲(Dipplin)係龍屬,但副牌
  係草(用草能量)。
- **裁決(只用真實數據)**:配色 + icon 改由**副牌實際嘅基本能量屬性**決定(`deckEnergyTypes`
  數 energy section 嘅基本能量,en「Grass Energy」+ zh「基本草能量」都認,特殊能量/
  能量回收 → null 跳過,按張數排序)。一屬=柔和底色;多屬=硬分色帶(火｜超｜惡),左邊
  主屬色。實測修正:厄鬼椪…大竺葵=草、祭典樂舞=草(唔再跟 Dipplin 龍)、多龍巴魯托ex=
  火/超/惡 分色、甲賀忍蛙=水。
- **能量屬性 icon**:每個系列 header 顯示該副牌用嘅能量屬性 TypeIcon(著返屬性色)。
- **誠實**:甲賀忍蛙副牌真實能量只有水(+Neo Upper 特殊),所以顯水;產品負責人預期嘅
...
## 2026-06-19 — 規則引擎卡效:神奇糖果暫不建模(進化鏈資料不足)+ 補兩張可靠搜尋

- **背景**:規則引擎(#36 第①步)逐張以「確切 catalog 效果文字」建模真實卡效。本輪原定做
  **神奇糖果(Rare Candy)** 跳階進化。
- **核實後的阻塞(只信真實數據)**:神奇糖果要驗證「基礎→1階→2階」整條進化鏈先可合法跳階,
  但 catalog 對進化鏈的編碼**不足且不可靠**:6645 張寶可夢中,2 階只有 88/566(約 16%)帶
  `evolveFrom`、`dexId` 約 41%;而真正 std 合法的進化線(例如整條噴火龍線)**兩者皆無**。
  dexId 係單體圖鑑編號,推斷唔到分支進化線。即係冇辦法唔靠估去確認「某 2 階可由某基礎進化」。
- **裁決**:**寧願唔建模,都唔出一個證唔到合法嘅跳階**(符合「絕不猜度、不自欺」最高標準)。
  神奇糖果列入 `COVERAGE.unmodeledKnown` 並附原因;沙盤照常可逐階手動進化。
- **正解(將來解鎖)**:為 catalog 補上**真實進化鏈資料**(可沿用現有 PokeAPI dex pipeline:
  species → evolution-chain),令 `evolveFrom`/進化線在 zh/ja/en 一致可查。屆時神奇糖果同
  「進化合法性嚴格驗證」皆可忠實建模(亦會順帶改善組牌工坊的進化鏈分組)。
- **本輪實際出貨(可靠、零新介面)**:沿用現有單選搜尋框架加入兩張**可靠**物品 ——
